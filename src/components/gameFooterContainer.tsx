...
export function FooterSection({ name, value }: { name: string, value: string }): JSX.Element {
    return (
...
export function GameFooterContainer(): JSX.Element {
        ...
        return (
            <footer className="w-full flex gap-6 p-6">
                {multiplayerMeta.players.map((player) => (
                    <FooterSection key={player.id} name={`P${player.id + 1}`} value={player.score.toString()} />
                ))}
            </footer>
        )
    }
    ...
    if (gameMeta.playerCount === 1 && soloMeta) {
        return (
            <footer className="w-full flex gap-6 p-6">
                <FooterSection name="Timer" value={soloMeta.timeElapsed.toString()} />
                <FooterSection name="Moves" value={soloMeta.movesTaken.toString()} />
            </footer>
        )
    }
